# Guia de Replicação — Fase02 (Pipeline ML Prescritivo)

> Documento-blueprint para **replicar o padrão arquitetural da Fase02 em outros projetos**.
...
## 0. O que é a Fase02 (em uma frase)

Pipeline de **manutenção prescritiva** sobre extrusoras industriais (regressão: `dias_até_próxima_manutenção`), executado em **8 estágios sequenciais idempotentes** que se comunicam **por arquivos em `outputs/`** (não por objetos em memória), orquestrados por um `run_pipeline.py` que importa cada estágio como módulo, com **histórico versionado** por execução, **detecção automática de mudanças** nos dados de entrada e **relatório PDF final** com versão auto-incrementada.

A receita abaixo é **agnóstica ao domínio**: troque "manutenção", "extrusora" e "dias até próxima manutenção" por qualquer outro contexto e a estrutura se mantém.
...
O `run_pipeline.py` muda o cwd para `outputs/` **antes** de chamar qualquer estágio. Isso permite que cada estágio escreva caminhos relativos curtos (`"data_raw.csv"`, `"models/"`) sem se preocupar com onde foi invocado. **Custo**: scripts **precisam** ser rodados a partir de `Fase02/scripts/`, e qualquer caminho de input fora de `outputs/` precisa ser absoluto (ou via `paths.py`).
...
| # | Stage SABO | Papel genérico | Entrada típica | Saída típica | Optional? |
|---|------------|----------------|----------------|--------------|-----------|
...
- **`normalize_extended_format(df, equip_name)`** para arquivos com schema diferente (ex.: `IJ-138.2.xlsx`). Cuidado com `Path("IJ-138.2").with_suffix(".csv")` — o `with_suffix` interpreta `.2` como extensão e gera nome errado. Use `parent / f"{base_name}.csv"`.
...
8. `add_maintenance_history_features(df)` e `add_preventive_history_features(df)` — features derivadas de planos de manutenção (SAP, RM.195).
...
11. **`aggregate_by_day_equipment(df)` (CRÍTICO)** — múltiplas ordens do mesmo dia/equipamento têm **o mesmo target** (ruído irredutível). Agregar reduz overfitting. Regras por coluna:
    ...
    - `drop`: metadados de linha (`Cód_Ordem`, `Fonte_Dados`)
...
- Padrões de scan: `IJ-*.xlsx`, `IJ-*.csv`, `DadosProducao*.xlsx`, `Dados Manut*.xlsx`. **Adapte o pattern ao seu domínio**, mas mantenha o conceito: lista explícita, não `*.*`.
...
- **`Path.with_suffix(".csv")` em arquivos com ponto no nome** (`IJ-138.2.xlsx`) silenciosamente trunca o nome. Sempre usar `parent / f"{stem}.csv"` quando o stem pode conter `.`.
- **Não rodar de outro diretório.** Como o orquestrador faz `os.chdir(OUTPUTS_DIR)` e importa estágios por nome via `sys.path`, executar de `Fase02/` em vez de `Fase02/scripts/` quebra os imports.
...
- Os nomes `IJ-*`, "extrusora", "manutenção", "cilindro/fuso" — são do domínio SABO. Adaptar.
- Os **valores hardcoded** de clamps (`0.60–1.20`, `0.70–1.30`) e fallback (`450 dias`) — calibrados para esse domínio. Recalibrar.
- O **sumário do PDF** (Capítulos 11–17, ordem específica) — é layout SABO. Reestruturar.
- A lista de equipamentos hardcoded em qualquer fallback — lê do XLSX de manutenção dinamicamente.
- Os patterns de scan (`IJ-*.xlsx`, `Dados Manut*.xlsx`) — substituir pelos do novo domínio.

---

## 12. Referências dentro deste repo

- Pipeline canônico: `sabo/sabo/Fase02/scripts/run_pipeline.py`
- Paths centrais: `sabo/sabo/Fase02/config/paths.py`
- Estágios: `sabo/sabo/Fase02/scripts/s00_*.py` … `s08_*.py`
- Histórico: `sabo/sabo/Fase02/outputs/history/{runs,reports,logs}/`
- Notas de R² baixo: `sabo/sabo/Fase02/MELHORIAS_MODELO_R2.md` (causas + fixes)
- Mudanças recentes: `sabo/sabo/Fase02/CHANGELOG.md`
- Padrões de relatório: `sabo/sabo/Fase02/scripts/s06_generate_report.py`
